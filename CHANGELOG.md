...
#### 1. 修复发图异常（大图导致上传失败）
**问题**: 原压缩策略阈值过高（12MB），VPS 到 QQ 服务器上传大图时频繁超时/失败  
**修复**: 重写 `_read_and_compress` 压缩逻辑

| 参数 | 旧版 | 新版 |
|------|------|------|
| 触发压缩阈值 | 12 MB | 500 KB |
| 最大尺寸 | 2048 px | 1600 px |
| 压缩策略 | 固定 quality=85 | 渐进式 (85→75→60→45→30) |
| 目标大小 | 无 | ≤ 400 KB |

**效果**: 图片体积控制在 400KB 以内，QQ 服务器上传成功率大幅提升
