...
| 功能 | 说明 |
|------|------|
| 🏷️ 分类发图 | 按图片目录分类发送，支持 `标签: 萝莉` / `看看腿` 等指定分类 |
| 🔍 多标签组合搜索 | 支持 AND 组合（如「少女,猫」= 同时包含两者），每标签独立缓存 |
| ⏰ 定时自动爬取 | 设定每天几点自动补充图片库存 |
| 📦 空库自动爬取 | 库存用完时自动触发补充，指定分类只补该分类，避免全量下载 |
...
| 🗜️ 渐进式压缩 | 超 500KB 自动压缩，目标 400KB 以内，确保 NapCat 稳定上传 |
...
1. 替换 `plugin.py` 和 `config.py` 为最新版本
